...
#class for storing the input parameters of the 2-dimensional fPSO model together
class fPSOparameters:
    #create an object (named for example inputParam) for all of the input parameters with the command
        # inputParam=fPSOparameters()
    ...
    def __init__(self):
        ...
        self.beta = -1.0 #popularity fading parameter; 0<beta<=1
        self.f = -1.0 #the initial radial coordinate of the lth node will be f*ln(l) in the native representation of the hyperbolic plane; 0<f
		#The absolute value of the exponent of the degree distribution:
			#At T<1: gamma = 1+2/(f*zeta*beta)
                        #At 1<T: gamma = 1+(2*T)/(f*zeta*beta)
                #The original PSO model uses f=2/zeta at T<1 and f=2*T/zeta at 1<T.
        self.T = -1.0 #temperature; 0<=T<1 or 1<T (T=0: the average clustering coefficient is maximized.)
    ...
    #the input parameters of a graph or graph list can be saved from a fPSOparameters object (named for example inputParam) with the command
    ...
    def save(self,directoryName):
        if self.numberOfGraphs==1:
            ...
            ReportFileHandler_readable = open(os.getcwd() + "/" + directoryName + "/fPSOparameters_readable"+self.graphID+".txt", 'w')
            #create a file, which is more appropriate for the loading functions:
            ReportFileHandler = open(os.getcwd() + "/" + directoryName + "/fPSOparameters"+self.graphID+".txt", 'w')
        elif 1<self.numberOfGraphs:
            if self.graphID!='':
                print('Error: The graph identifier can be customized only if 1 graph is generated!')
            else:
                #save the input parameters in a readable way:
                ReportFileHandler_readable = open(os.getcwd() + "/" + directoryName + "/fPSOparameters_readable.txt", 'w')
                #create a file, which is more appropriate for the loading functions:
                ReportFileHandler = open(os.getcwd() + "/" + directoryName + "/fPSOparameters.txt", 'w')
        else:
    ...
    #the input parameters of a previously saved graph (with a string identifier IDofTheGraph) or graph list can be loaded into a fPSOparameters object (named for example inputParam) with the command
    ...
    def load(self,directoryName,graph_ID=''):
        if graph_ID != '': #load the parameters of one graph denoted by a previously given graph identifier
            ...
            ReportFileHandler=open(os.getcwd()+"/"+directoryName+"/fPSOparameters"+self.graphID+".txt",'r')
            ...
            ReportFileHandler=open(os.getcwd()+"/"+directoryName+"/fPSOparameters.txt",'r')
            listOfLines=ReportFileHandler.readlines()
            ReportFileHandler.close()
            self.zeta=float(listOfLines[0])
            self.N=int(listOfLines[1])
...
#The file name always contains a graph identifier (if only one graph is saved, the identifier is the one given by the user in the corresponding fPSOparameters object; for a graph list each file name contains one of the integers 0,1,2,...,numberOfGraphs-1).

#inputParameters=fPSOparameters object with the input parameters of the model
#listToSave=list of lists, where each list contains the cutoff distance of the connection probability in each time step for a given graph. (In one listToSave object all the corresponding graphs' input parameters are given by the same fPSOparameters object.) When saving only one list named givenList, set the argument listToSave to [givenList].
...
#The file name always contains a graph identifier (if one graph is saved, the identifier is the one given by the user in the corresponding fPSOparameters object; for a graph list each file name contains one of the integers 0,1,2,...,numberOfGraphs-1).
...
#inputParameters=fPSOparameters object with the input parameters of the model
#GraphList=A list of the created NetworkX Graphs. (In one list all the graphs' input parameters are given by the same fPSOparameters object.) When saving only one graph (e.g. G), set the argument GraphList to [G].
...
def saveCoordinates(inputParameters,radialCoordList,CartCoordList,fileName,directoryName):
    if inputParameters.numberOfGraphs==1: #radialCoordList is a list containing only one NumPy array and CartCoordList is a list containing only one NumPy array
        ...
        for j in range(inputParameters.N):
            ...
            fileHandler.write(str(CartCoordList[0][j,0])+"\t") #the first Cartesian coordinate of the jth node
            fileHandler.write(str(CartCoordList[0][j,1])+"\n") #the last (=the second) Cartesian coordinate of the jth node
        fileHandler.close()
        ...
        for i in range(inputParameters.numberOfGraphs):
            ...
            for j in range(inputParameters.N):
                ...
                fileHandler.write(str(CartCoordList[i][j,0])+"\t") #the first Cartesian coordinate of the jth node
                fileHandler.write(str(CartCoordList[i][j,1])+"\n") #the last (=the second) Cartesian coordinate of the jth node
            fileHandler.close()
    else: #1<inputParameters.numberOfGraphs and inputParameters.graphID!=''
        print('Error: The graph identifier can be customized only if 1 graph is generated!')
...
def saveAll(inputParameters,Rlist,GraphList,isWeighted,radialCoordList,CartCoordList,directoryName):
    ...
    if inputParameters.T<0 or inputParameters.T==1:
...
def load_Rlist(directoryName,fileName,inputParameters):
    ...
    elif 1<inputParameters.numberOfGraphs and inputParameters.graphID=='': #load the data for a list of NetworkX Graphs created using the parameters given by the same fPSOparameters object, i.e. by inputParameters
...
def loadGraphList_edgeList(directoryName,inputParameters,isWeighted):
    ...
    elif 1<inputParameters.numberOfGraphs and inputParameters.graphID=='': #load a list of NetworkX Graphs created using the parameters given by the same fPSOparameters object, i.e. by inputParameters
...
def loadCoordinates(directoryName,fileName,inputParameters):
    ...
    Coords=np.zeros((inputParameters.N,2)) #Cartesian coordinates of the nodes in one graph
...
#if isWeighted==True, than weights will be assigned to the edges, otherwise not
#Use whichGraph='' (default) to load a whole graph list from the directory named directoryName. (In one list all the corresponding graphs' input parameters are given by the same fPSOparameters object.)
#To load a graph with a customized identifier (and thus with a customized fPSOparameters object), set whichGraph to the string identifier of the desired graph.
def load_fPSOgraph(directoryName,isWeighted,whichGraph=''):
    params = fPSOparameters() #create a fPSOparameters object for storing the model parameters
    ...
    else: #0<params.T<1 or 1<params.T
