...
expectedGamma = 1.5 #the desired degree decay exponent
if iP.T<1:
    iP.f = 2/(iP.zeta*iP.beta*(expectedGamma-1))
else: #1<iP.T
    iP.f = 2*iP.T/(iP.zeta*iP.beta*(expectedGamma-1))
...
[Rlist,GraphList,radialCoordList,CartCoordList] = fPSO.fPSO(inputParameters=iP,directoryName=directoryName_1ex,numORan='an',isWeighted=False) #the directory named directoryName_1ex will be created here, if it has not already been created
...
[iP_loaded,Rlist_loaded,GraphList_loaded,radialCoordList_loaded,CartCoordList_loaded] = ds.load_fPSOgraph(directoryName=directoryName_1ex,isWeighted=False,whichGraph=graphName_1ex) #Note that in the case of T=0, Rlist_loaded is just an empty list, since it is not needed for the network generation, as each node connects to the m number of hyperbolically closest nodes at T=0.
...
[Rlist,GraphList,radialCoordList,CartCoordList] = fPSO.fPSO(inputParameters=iP,directoryName=directoryName_exList1,numORan='an',isWeighted=False) #the directory named directoryName_exList1 will be created here, if it has not already been created
    #if numORan is 'num', then the cutoff distance R of the connection probability is calculated for each node by solving numerically Equation (S2.9) of the Supplementary Information, which expresses that the expected number of nodes connecting to the new node at its arrival is equal to m
    #if numORan is 'an', then the cutoff distance R of the connection probability is calculated faster, using the approximating formulas given by Equations (S2.11) and (S2.16) of the Supplementary Information
    #If isWeighted is True, then a weight calculated as 1/(1+hyperbolic distance between the two nodes in question) is assigned to each link at the end of the network growth. Otherwise, no weight is assigned to the links.
...
[iP_loaded,Rlist_loaded,GraphList_loaded,radialCoordList_loaded,CartCoordList_loaded] = ds.load_fPSOgraph(directoryName=directoryName_exList1,isWeighted=False) #Note that in the case of T=0 Rlist_loaded is just an empty list, since it is not needed for the network generation.
...
for g in range(numOfGraphs):
    ...
    [Rlist,GraphList,radialCoordList,CartCoordList] = fPSO.fPSO(inputParameters=iP,directoryName=directoryName_exList2,numORan='an',isWeighted=False) #the directory named directoryName_exList2 will be created here, if it has not already been created
        #if numORan is 'num', then the cutoff distance R of the connection probability is calculated for each node by solving numerically Equation (S2.9) of the Supplementary Information, which expresses that the expected number of nodes connecting to the new node at its arrival is equal to m
        #if numORan is 'an', then the cutoff distance R of the connection probability is calculated faster, using the approximating formulas given by Equations (S2.11) and (S2.16) of the Supplementary Information
        #If isWeighted is True, then a weight calculated as 1/(1+hyperbolic distance between the two nodes in question) is assigned to each link at the end of the network growth. Otherwise, no weight is assigned to the links.
    #the list with the cutoff distance of the connection probability in each time step is Rlist[0]
...
for g in range(numOfGraphs):
    [iP_loaded,Rlist_loaded,GraphList_loaded,radialCoordList_loaded,CartCoordList_loaded] = ds.load_fPSOgraph(directoryName=directoryName_exList2,isWeighted=False,whichGraph=graphNameList[g]) #Note that in the case of T=0 Rlist_loaded is just an empty list, since it is not needed for the network generation.
    #Here you can do what you want with the list Rlist_loaded[0], the NetworkX Graph GraphList_loaded[0], the NumPy array radialCoordList_loaded[0] and/or the N-by-d NumPy array CartCoordList_loaded[0].
