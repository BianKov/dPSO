...
def dPSO(inputParameters,directoryName,numORan,isWeighted):
    if inputParameters.T==0: #deterministic edge formation
        [Rlist,GraphList,radialCoordList,CartCoordList] = dPSO_T0(inputParameters.d, inputParameters.zeta, inputParameters.N, inputParameters.m, inputParameters.beta, inputParameters.numberOfGraphs, isWeighted) #note that Rlist is empty for T==0
    ...
    elif inputParameters.T<(1/(inputParameters.d-1)):
        [Rlist,GraphList,radialCoordList,CartCoordList] = dPSO_Tnot0_belowLimit(inputParameters.d, inputParameters.zeta, inputParameters.N, inputParameters.m, inputParameters.beta, inputParameters.T, inputParameters.numberOfGraphs, numORan, isWeighted)
        ds.saveAll(inputParameters,Rlist,GraphList,isWeighted,radialCoordList,CartCoordList,directoryName) #save the edge list and the node coordinates for all the generated networks
        return [Rlist,GraphList,radialCoordList,CartCoordList]

    elif (1/(inputParameters.d-1))<inputParameters.T:
        [Rlist,GraphList,radialCoordList,CartCoordList] = dPSO_TaboveLimit(inputParameters.d, inputParameters.zeta, inputParameters.N, inputParameters.m, inputParameters.beta, inputParameters.T, inputParameters.numberOfGraphs, numORan, isWeighted)
        ds.saveAll(inputParameters,Rlist,GraphList,isWeighted,radialCoordList,CartCoordList,directoryName) #save the edge list and the node coordinates for all the generated networks
        return [Rlist,GraphList,radialCoordList,CartCoordList]
...
def dPSO_T0(d,zeta,N,m,beta,numberOfGraphs,isWeighted):
...
def dPSO_Tnot0_belowLimit(d,zeta,N,m,beta,T,numberOfGraphs,numORan,isWeighted):
    ...
    Rvalues = cutoffDists(d,zeta,N,m,beta,T,numORan) #the cutoff distance of the connection probability for node i is Rvalues[i]
...
def dPSO_TaboveLimit(d,zeta,N,m,beta,T,numberOfGraphs,numORan,isWeighted):
    Rlist = [] #Rlist[g]=list with the cutoff distance of the connection probability in each step of the growth of the gth graph
    GraphList = [] #list of the generated NetworkX Graphs
    radialCoordList = [] #radialCoordList[g]=NumPy array with the radial coordinates of the gth graph's nodes in the native representation of the d-dimensional hyperbolic space with curvature -zeta^2
    CartCoordList = [] #CartCoordList[g]=N-by-d NumPy array containing the Cartesian coordinates describing the positions of the gth graph's nodes in the native representation of the d-dimensional hyperbolic space with curvature -zeta^2
    ...
    for g in range(numberOfGraphs):
        ...
        G = nx.Graph() #the initial empty network
        r = np.zeros(N) #radial coordinates of the N nodes in the native representation of the d-dimensional hyperbolic space with curvature -zeta^2
        r_initial = (2*T*(d-1)/zeta)*np.log(np.linspace(1,N,N)) #initial radial coordinates of the N nodes ((2*T/zeta)*ln(timestep)) in the native representation
        Coords = np.zeros((N,d)) #Cartesian coordinates of the N nodes in the native representation of the d-dimensional hyperbolic space with curvature -zeta^2
...
#numORan is a string specifying what type of calculations will be used:
    #if numORan=='num', the cutoff distances will be calculated by solving the equation m=i*P(i) numerically
    #if numORan=='an', the cutoff distances will be calculated analytically, using approximating formulas
def cutoffDists(d,zeta,N,m,beta,T,numORan):
    if numORan=='num':
